...
var forCoins_counter = 0

signal winState(value)

func _ready():
	GameManager.current_lvl = 1
...
func _process(delta):
	#Turn the collision for the coins to reach the player
	if forCoins_counter >= 30:
		$ForCoins.set_collision_layer_bit(0, true)
		$ForCoins.set_collision_mask_bit(0, true)
